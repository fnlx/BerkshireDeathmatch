...
function TeamDeathmatch.createMap()
	TeamDeathmatch["Teams"][1] = 0;
	TeamDeathmatch["Teams"][2] = 0;
...
function sortPlayer(player)
	if(TeamDeathmatch["Teams"][1] == TeamDeathmatch["Teams"][2])then
		local rnd = math.random(1,2);
		if(rnd == 1)then
			player:setData("Team",1);
			TeamDeathmatch["Teams"][1] = TeamDeathmatch["Teams"][1] + 1;
		else
			player:setData("Team",2);
			TeamDeathmatch["Teams"][2] = TeamDeathmatch["Teams"][2] + 1;
		end
	elseif(TeamDeathmatch["Teams"][1] < TeamDeathmatch["Teams"][2])then
		player:setData("Team",1);
		TeamDeathmatch["Teams"][1] = TeamDeathmatch["Teams"][1] + 1;
	elseif(TeamDeathmatch["Teams"][2] < TeamDeathmatch["Teams"][1])then
		player:setData("Team",2);
		TeamDeathmatch["Teams"][2] = TeamDeathmatch["Teams"][2] + 1;
	end
	...
	if(client:getData("Lobby") == "Eingangshalle")then
		...
		setTimer(function(client)
			if(isElement(client))then
				...
				outputChatBox("[INFO]: Mit dem Befehl /leave kannst du die Lobby wieder verlassen.",client,200,200,0);
			end
		end,3500,1,client)
	end
end)
