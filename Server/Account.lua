...
addCommandHandler("oldAccount",function(player,cmd,name,password)
	if(name and password)then
		...
		if(#result >= 1)then
			...
			outputChatBox("Die Serial deines Accounts wurde aktualisiert. Ab sofort wirst du automatisch eingeloggt.",player,0,125,0);
			setPlayerDatas(player);
...
addCommandHandler("newAccount",function(player,cmd,password)
	if(password)then
		...
		if(#result == 0)then
			...
			outputChatBox("Dein Account wurde angelegt. Ab sofort wirst du automatisch eingeloggt.",player,0,125,0);
			setPlayerDatas(player);
